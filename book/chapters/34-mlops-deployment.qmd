...
def make_app(pipeline, onnx_sess, feature_names):
    ...
    def score(req: PredictRequest):
        x = np.asarray(req.features, dtype=np.float32)
        from csutils import scorecard_points
        p = pipeline.predict_proba(x)[:, 1]
