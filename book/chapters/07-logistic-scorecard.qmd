...
2.  *Separation.* Rare monotone bins (e.g., a PAY_0 bin with zero goods) make the likelihood diverge. Optimal binning enforces a minimum bad and good rate per bin [@navas2016optimal] to prevent this before fitting.
...
The OOT-selected `C` is usually equal to or smaller than the 1-SE choice, because vintage shift punishes models that leaned on training-window quirks. The "take the larger $\lambda$" recipe in the prose is the conservative read: deploy the more regularized of the two.
...
With `pd_uncal`, `pd_platt`, and `pd_iso` already in scope from the demo above, we can encode the calibration decision tree from @fig-ch07-calibration-decision as a function and apply it to a handful of representative scenarios.

```{python}
#| label: tbl-ch07-calibration-recommendations
#| tbl-cap: "Calibration-method recommendations from @fig-ch07-calibration-decision applied to five scenarios, with held-out Brier on Taiwan default after applying the recommendation. The Brier (no calib) column is the same baseline across rows; compare it against Brier (held-out) to read off the calibrator's effect."
...
base_brier = brier_score_loss(y_test, pd_uncal)

def brier_for(rec):
    if rec in ("No calibration",) or rec.startswith("King-Zeng"):
        return base_brier
    if rec == "Platt":
        return brier_score_loss(y_test, pd_platt)
    if rec == "Isotonic":
        return brier_score_loss(y_test, pd_iso)
    if rec == "Beta":
        # No native sklearn impl in this demo; Platt is the practical fallback.
        return brier_score_loss(y_test, pd_platt)
    return float("nan")
...
for s in scenarios:
    ...
    rows.append({"scenario": s["name"], "recommendation": rec,
                 "Brier (held-out)": round(brier_for(rec), 4),
                 "Brier (no calib)": round(base_brier, 4)})
calib_recs = pd.DataFrame(rows)
print(calib_recs.to_string(index=False))
```

Two patterns are visible. First, the "Big retail LR" row recommends *no calibration* and lands at the same Brier as the uncalibrated model by construction; this is the regime where Platt would change essentially nothing. Second, the "Tree ensemble" rows recommend Platt or isotonic, and on Taiwan both move Brier in the right direction (compare *Brier (held-out)* against *Brier (no calib)*); the small-calibration-set row falls back to Platt, which is the safer choice when isotonic would over-fit. The function and table together turn the verbal decision tree into an auditable artifact, which is what an SR 11-7 validator will ask for.
