...
Differentiating @eq-loglik term by term,
$$
\frac{\partial \ell}{\partial \beta} = \sum_{i=1}^{n} (y_i - \pi_i) x_i = X^\top (y - \pi),
$$ {#eq-score}
$$
...
= -X^\top W X.
$$ {#eq-hessian}
The Hessian in @eq-hessian is negative semi-definite, which confirms concavity. Observed and expected Fisher information coincide, a consequence of the canonical link.
...
$$ {#eq-irls}
@eq-irls is the iteratively reweighted least squares (IRLS) form [@green1984iteratively; @nelder1972generalized]. Each iteration is a WLS regression of $z$ on $X$ with weights $W$. Convergence is quadratic once you are close, and damping (step halving) handles the rare divergent early steps.
...
3. *Numerical overflow in the sigmoid.* Large $|\eta|$ causes `exp(eta)` to overflow. Any production IRLS should use the branchless stable form `np.where(eta >= 0, 1/(1+exp(-eta)), exp(eta)/(1+exp(eta)))` shown in the implementation below.
...
from csutils import load_german_credit, load_taiwan_default, ks_statistic, gini, psi, scorecard_points
...
firms["O"] = (c["intercept"]
              ...
              + c["CHIN"]*firms["CHIN"])
firms["PD_1y"] = 1.0 / (1.0 + np.exp(-firms["O"]))
...
pred_sm = 1 / (1 + np.exp(-Xte @ sm_logit.params))
...
The `csutils.psi` helper computes the score-level PSI; we will compute CSI below.
...
@bartlett2022consumer and @hurlin2024fairness give the up-to-date empirical view on fintech-era lending discrimination. A logistic regression on legitimate features can still produce disparate outcomes; @sec-ch24's fairness audit is mandatory before go-live.
...
Macro volatility is not optional. @chu2024vietnam show that Vietnamese bank credit responds to uncertainty shocks more strongly than in advanced markets, which means scorecard PD tracks a moving ground truth. Vietnamese GDP swings and property-cycle episodes (2012, 2022) are documented in IMF Article IV filings [@imf2023vietnamart4]. Seasonality is the other first-order effect. Tet bonuses, rural-urban remittances, and closing wholesale markets produce repeatable Q1 liquidity compression that a fixed-threshold scorecard reads as a risk spike unless explicitly adjusted [@leboulanger2021tet].
