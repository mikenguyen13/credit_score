...
p_default = 1 / (1 + np.exp(2.0 + 1.5 * X))
...
$$
\widehat{\theta}_{\text{DML}} = \left( \frac{1}{n} \sum_{k=1}^{K} \sum_{i \in I_k} \widetilde{D}_i^{ 2} \right)^{-1} \frac{1}{n} \sum_{k=1}^{K} \sum_{i \in I_k} \widetilde{D}_i \widetilde{Y}_i,
$$ {#eq-dml-theta}
...
pd_hat = 1 / (1 + np.exp(-(0.5 + 1.0 * X)))  # perfectly calibrated here
...
$$
\widehat{\tau}_{\text{AIPW}} = \frac{1}{n} \sum_{i=1}^{n} \left[ \widehat{\mu}_1(X_i) - \widehat{\mu}_0(X_i) + \frac{D_i (Y_i - \widehat{\mu}_1(X_i))}{\widehat{e}(X_i)} - \frac{(1 - D_i)(Y_i - \widehat{\mu}_0(X_i))}{1 - \widehat{e}(X_i)} \right],
$$ {#eq-aipw}
...
def true_pd(X):
    return 1 / (1 + np.exp(-(true_intercept + X @ true_beta)))
...
- @goodmanbacon2021difference, @callaway2021difference, @roth2023what on modern DiD.
