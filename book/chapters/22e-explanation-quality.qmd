...
sv = explainer.shap_values(Xte.iloc[:200])
...
def shap_of(x):
    return explainer.shap_values(pd.DataFrame([x], columns=X.columns))[0]
...
explainer_int = shap.TreeExplainer(clf, data=Xtr.sample(100, random_state=SEED), feature_perturbation='interventional')
sv_int = explainer_int.shap_values(Xte.iloc[:50])
overlaps = [top_k_overlap(sv[i], sv_int[i], k=5) for i in range(50)]
print(f"TreeSHAP(observational) vs TreeSHAP(interventional) top-5 Jaccard: mean={np.mean(overlaps):.3f}, min={np.min(overlaps):.3f}")
```

Low Jaccard is itself a signal: it means the observational-vs-interventional choice is consequential for these applicants and the model card should disclose which was used.
...
$$
\min_{\delta} \lambda_{\mathrm{fit}} \cdot \big(f_{t'}(x + \delta) - \max_{k \neq t'} f_k(x + \delta) + \kappa\big)^+ + \beta \|\delta\|_1 + \|\delta\|_2^2 + \gamma \cdot \mathrm{AE\_loss}(x + \delta),
$$ {#eq-cem}
...
$$
P = \arg\max_{P \subseteq \mathcal{D}} \mathrm{MMD}^2(\mathcal{D}, P), \qquad C = \arg\max_{C \subseteq \mathcal{D}} \sum_{x \in C} \|\hat\rho(x) - \rho_P(x)\|_1,
$$ {#eq-mmd-critic}
