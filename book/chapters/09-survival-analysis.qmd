...
The benchmark on UCI German credit at @sec-ch09-benchmark scores nine families on one dataset. Useful, but it answers only the question "which model wins on this file?". Two questions a model-risk reviewer asks before signing off are upstream of that:

1.  **What does each family lock in by assumption?** A Cox PH model assumes proportional hazards. A Weibull AFT assumes a monotone hazard. A Random Survival Forest assumes nothing about hazard shape but cannot extrapolate past the longest training time. A Shumway logit assumes the period basis spans the seasoning curve. The right way to read a benchmark is with the assumption sheet open beside it.
...
This section answers both. First, a static assumption matrix for every family covered in the chapter. Then a controlled stress benchmark: five synthetic worlds, one common roster, three metrics, one heatmap. Each world targets exactly one assumption, so the deviation from the oracle isolates which family handles which violation.
...
A few observations from the matrix that show up later in the stress benchmark. The Cox family handles every lever **except** parametric extrapolation cleanly, but always with an extension. The AFT family is the only family that gives lifetime PD with no extension, but only the parametric shape it commits to. The cure model is the only family that handles a long-run immune fraction natively. Fine-Gray is the only single-fit family that gives a calibrated cumulative incidence function under competing risks. The tree ensembles win on flexibility and lose on extrapolation, which is the trade an IFRS 9 lifetime ECL pipeline cannot ignore.

### Stress benchmark: five worlds, one roster {#sec-ch09-comparison-stress}

Five synthetic data-generating processes (DGPs), each violating exactly one structural assumption that one or more families rely on. The roster is a tight set chosen to span the families in the matrix: Kaplan-Meier (marginal baseline), Cox PH linear, Weibull AFT, LogNormal AFT, Random Survival Forest, gradient-boosted survival, Shumway discrete logit. We add one specialist per relevant DGP: Fine-Gray for the competing-risk world, mixture cure for the cure world. The roster is fit on a 70/30 stratified holdout of each DGP, and the same three-metric scoring (C-index, integrated Brier score over horizons 6 to 48 months, calibration deviation at 24 months against the oracle survival function) is applied uniformly.
...
def _fit_roster(tr, te, dgp_name):
    ...
    # Specialist: Fine-Gray on competing risks via Aalen-Johansen marginal CIF
    if dgp_name.startswith('C.') and 'cause' in tr.columns:
        try:
            ...
            out['Aalen-Johansen (def)'] = (np.tile(S_def[None, :], (n_te, 1)), times)
        except Exception: pass
    # Specialist: simple Weibull cure EM on cure DGP (closed-form approximation)
...
@fig-ch09-comparison-heatmap puts the three metrics on one panel each. Lower is better in the right two panels (IBS, portfolio-level marginal calibration error at 24 months). Higher is better in the left panel (C-index). White cells are families that do not exist for that DGP (the cure specialist for the PH worlds, Aalen-Johansen for the non-competing-risk worlds). The marginal-calibration metric averages the predicted and oracle cumulative incidences across the test fold and reports the absolute gap, so it is the right question for portfolio-level provisioning. Per-borrower MAE is more sensitive to discrimination but conflates with the C-index panel.

```{python}
#| label: fig-ch09-comparison-heatmap
#| fig-cap: "Side-by-side stress benchmark: rows are model families, columns are the five synthetic DGPs (A: Weibull PH baseline; B: PH violation with sign flip at 12 months; C: competing risks; D: cure mixture with 40 percent immune; E: left truncation with delayed entry to 18 months). Left panel: C-index on the held-out 30 percent test fold, higher is better. Centre: integrated Brier score over 6 to 48 months, lower is better. Right: portfolio-level marginal calibration deviation at 24 months, defined as $|\\bar F_{\\text{pred}}(24) - \\bar F_{\\text{oracle}}(24)|$ averaged across the test fold; this is the IFRS 9 reviewer's question and lets KM and Aalen-Johansen (which do not condition on covariates) score honestly even though their C-index is 0.5. White cells are families that do not have a fit for that DGP. The patterns: every PH-compatible family ties on DGP A; the parametric families lose long-horizon shape on DGP B (visible in @fig-ch09-comparison-terms); KM and Cox PH overshoot the marginal default cumulative on DGP C while Aalen-Johansen tracks it; AFTs miss the plateau on DGP D while the mixture cure recovers it; every estimator that ignores delayed entry overshoots the early hazard on DGP E."
...
model_order = ['Kaplan-Meier', 'Cox PH', 'Weibull AFT', 'LogNormal AFT',
               'RSF', 'GB Survival', 'Shumway logit',
               'Aalen-Johansen (def)', 'Mixture cure']
...
#| fig-cap: "Term-structure forecast for one high-risk test borrower (90th percentile of the covariate $x$) under each DGP. Black dashed: oracle cumulative PD computed analytically from the simulator. Coloured solid: each fitted model's predicted cumulative PD. The five panels read: (A) every PH-compatible family tracks the oracle. (B) Cox PH and Weibull AFT lock onto the early-life slope and miss the long-horizon plateau because they cannot represent a sign flip; the Shumway period basis and the tree ensembles bend with the data. (C) the marginal Kaplan-Meier and the Cox PH cause-specific fit overshoot the default cumulative because they treat prepayment as censoring; Aalen-Johansen recovers the marginal default cumulative. (D) the AFTs run past the plateau because no parametric Weibull/LogNormal hazard goes to zero in finite time; the mixture cure recovers the long-run immune fraction. (E) every standard estimator overshoots the early hazard because the delayed-entry rows are absent from the at-risk set the simulator used."

fig, axes = plt.subplots(1, 5, figsize=(20.0, 4.2), sharey=True)
cmap_t = plt.get_cmap('tab10')
plot_models = ['Cox PH', 'Weibull AFT', 'LogNormal AFT', 'RSF',
               'GB Survival', 'Shumway logit', 'Aalen-Johansen (def)', 'Mixture cure']
...
for ax, dgp_name in zip(axes, dgp_order):
    ...
    for k_idx, nm in enumerate(plot_models):
        ...
        S_te, ts = S_store[key]
        ax.plot(ts, (1 - S_te[i_hi]) * 100, lw=1.1, color=cmap_t(plotted % 10), label=nm)
        plotted += 1
...
Five things the heatmap and the term-structure overlay together say:

-   **Cox PH and Weibull AFT win on DGP A and only DGP A.** When the data are PH-clean, the lowest-variance estimator is the parametric one. Every additional flexibility (RSF, GBS, Shumway period basis) pays a small variance cost without recovering bias because there is no bias to recover.
-   **PH violation hides in the C-index.** On DGP B, Cox PH and Weibull AFT lose only a small amount of C-index relative to the tree ensembles, but the term-structure overlay at @fig-ch09-comparison-terms shows the parametric families locking onto the early-life slope and missing the long-horizon plateau. This matches the field experience: PH violations are quiet at single-horizon discrimination and loud at lifetime-PD shape, which is what an IFRS 9 stage-2 / lifetime backtest reads.
-   **Competing risks is the largest assumption-violation cost in the chapter.** On DGP C, the marginal KM and the Cox cause-specific overshoot the default cumulative by a factor that no Brier-or-AUC tuning will close. Aalen-Johansen and Fine-Gray are not "nice-to-have"; they are the **only** roster members that produce a calibrated cumulative incidence on a portfolio with prepayment.
-   **AFT tails do not plateau.** On DGP D, the LogNormal and Weibull AFTs run smoothly past the immune plateau and toward $1 - S(t \mid x) \to 1$ at long horizons. The mixture cure is the only single-fit estimator that respects the long-run immune fraction. On a real consumer book this is the difference between a reasonable and an over-stated lifetime ECL.
...
The takeaway is the matrix at @sec-ch09-comparison-matrix used in the order it implies. Inspect the data first (Schoenfeld residual, prepayment fraction, immune fraction at the longest observed age, delayed-entry distribution at vintage open). Then pick the family whose assumption sheet matches what the data are actually doing. The benchmark on UCI German credit at @sec-ch09-benchmark scores model fit on one file; the heatmap above scores model fit on five worlds with controlled violations and is the artifact a model-risk reviewer can read in 30 seconds.
