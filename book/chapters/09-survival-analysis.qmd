...
Survival is a family of models, not a single estimator. Each member of the family relaxes a structural assumption that an earlier member relied on, and pays for that flexibility somewhere else (more data, more compute, weaker extrapolation, harder identification). @fig-ch09-genealogy is the map. Read the edges as "what this step buys"; read the assumption matrix at @sec-ch09-comparison-matrix as "what it costs". The stress benchmark at @sec-ch09-comparison-stress drops the whole roster onto six controlled DGPs and shows the cost empirically.
...
Five operational additions, in order of payoff for a credit production stack. @fig-ch09-extension-selector walks the choice from a fitted Cox or Weibull baseline; the numbered list below records the operational note and the section pointer for each leaf.
...
The expanded benchmark fits **seventeen** families spanning four groups. *Classical statistical*: (i) Cox PH linear, (ii) Cox PH with natural cubic splines, (iii) Cox PH stratified on `purpose`, (iv to vi) Weibull / log-logistic / log-normal AFT, (vii) hand-rolled exponential AFT. *Marketing duration models* (@sec-ch09-marketing): (viii) single-event Weibull mixture cure (@sec-ch09-cure), (ix) gamma-frailty Weibull with `purpose` as cluster key (@sec-ch09-frailty), (x) latent-class piecewise-exponential mixture (@sec-ch09-latent-class), (xi) shifted Beta-Geometric retention (@sec-ch09-sbg). *Discrete-time*: (xii) Shumway logit (@sec-ch09-shumway), (xiii) cloglog grouped-data hazard (the discrete analog of Cox PH). *Machine-learning challengers*: (xiv) Random Survival Forest [@ishwaran2008random], (xv) sksurv gradient-boosted survival with Cox loss [@chen2016xgboost], (xvi) XGBoost long-table classifier [@tian2015variable], and (xvii) DeepSurv [@katzman2018deepsurv] (graceful skip if `pycox` is not installed; the cell is reported as `n/a` in the table). The multi-event mixture cure is out of scope on UCI German because the file has no prepayment indicator (the synthetic Vietnam-Tet panel at @sec-ch09-vietnam-code closes that gap with a Fine-Gray and multi-event-cure end-to-end). The Shumway state-of-the-art layers that need market-equity, macro, or calendar covariates (CHS layer 1, Duffie stochastic-covariate layer 2, filtered frailty / Bharath naive distance-to-default layer 3) are exercised on the controlled stress benchmark in @sec-ch09-comparison and on the production panel in @sec-ch09-shumway-layers-code rather than here, since UCI German carries no equity or calendar series. State dependence and dynamic-promotion long-table extensions (@sec-ch09-state-dep) require a per-loan history that UCI German does not carry; they are scored on the synthetic Vietnam panel.
...
This section answers both. First, a static assumption matrix for every family covered in the chapter. Then a controlled stress benchmark: five synthetic worlds, one common roster, three metrics, one heatmap. Each world targets exactly one assumption, so the deviation from the oracle isolates which family handles which violation. The matrix is the cost side of the genealogy at @fig-ch09-genealogy: each row in the matrix is a node in the tree, each column is the assumption an arrow into that node relaxes.
...
-   Kaplan-Meier for curves, Cox PH for semiparametric regression, parametric AFT for lifetime projection (@sec-ch09-km-cox, @sec-ch09-aft). Each has a role. Cox has no baseline assumption but cannot forecast beyond observed times without extra work; parametric AFTs project freely but impose a hazard shape.
-   Competing risks (prepayment vs default) are standard in retail (@sec-ch09-competing). Cause-specific hazards for mechanism, Fine-Gray for cumulative incidence, Aalen-Johansen for nonparametric curves.
-   Mixture cure models matter when a material fraction of borrowers never default within the horizon (@sec-ch09-cure). Fit with EM. The flattening Kaplan-Meier is the diagnostic.
-   Vintage analysis is the portfolio-level view of the same machinery (@sec-ch09-vintage). Age, vintage, and calendar effects are identifiable only with a stack of cohorts.
-   Shumway's multi-period logistic is the discrete-time hazard (@sec-ch09-shumway). The long table makes it embarrassingly parallel and reuses any existing logistic-regression codebase.
-   Scale survival: Kaplan-Meier runs on window functions; Cox needs broadcast reductions or a switch to the Shumway long form; parametric AFTs distribute trivially (@sec-ch09-scalability).
