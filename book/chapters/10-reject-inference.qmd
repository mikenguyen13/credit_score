...
The funnel is descriptive. The substantive damage is visible on a default-rate curve. @fig-ch10-conditional-shift shows what happens when we draw the same plot using the full applicant population (which we know only because this is a simulation) and using the accepted slice (which is all a real lender ever sees).
