...
Stage 1 recovers $\gamma$ accurately. Stage 2 recovers $\beta$ and the IMR coefficient recovers $\rho$ (under the probit-probit normalization $\sigma = 1$). The Heckman estimates are close to the oracle, while the naive estimates are visibly biased. This is the mechanical gain from correctly modelling $\mathbb{E}[U \mid S=1]$.
...
```{python}
from csutils import load_taiwan_default, train_valid_test_split, ks_statistic, gini
