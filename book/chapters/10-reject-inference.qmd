...
%%| fig-cap: "Selection pipeline as a funnel from through-the-door applicant to observed outcome $Y$. Stages carry the moderators that bind there: M1 covariate shift on $X$ ($P(X \\mid S{=}1) \\neq P(X)$); M2 selection on unobservables ($\\mathrm{Corr}(U,V) \\neq 0$); M3 policy overlap ($P(S{=}1 \\mid x) > 0$ everywhere); M4 exclusion restriction (a $Z$ that shifts $S$ but not $Y$); M5 vintage and macro state (through-the-cycle versus point-in-time); M6 bureau product gap (limit, rate, servicer differ from the lender's product); M7 within-reject coverage (10 to 30 percent of rejects have no bureau trade-line). Red nodes mark paths where $Y$ is missing for the lender. Subsequent sections take the moderators one at a time."
flowchart TD
    ...
    S1["Stage 1: channel and eKYC screen<br/><b>[M3]</b>"]:::stage
    L1["Filtered pre-score<br/>X often not stored"]:::miss
    S2["Stage 2: policy and underwriter decision<br/><b>[M1, M2, M3, M4]</b>"]:::stage
    REJ["Rejected (S=0)<br/>Y missing for the lender"]:::miss
    YB["Bureau outcome Y_B<br/>different lender, different product<br/><b>[M6, M7]</b>"]:::miss
    ACC["Funded (S=1)<br/>limit, rate, term"]:::stage
    PW["Stage 3: performance window<br/>vintage v, macro state m, <b>[M5]</b>"]:::stage
