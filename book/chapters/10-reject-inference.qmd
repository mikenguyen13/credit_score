...
#| fig-cap: "Two faces of the selection problem on a synthetic through-the-door population (n = 8,000) with bivariate-normal correlation $\\rho = 0.6$ between the latent default error and the latent acceptance error. (a) Covariate shift: the accepted-population density of the feature $X$ is pulled toward the safer (lower-$X$) side because the policy declines high-$X$ applicants. (b) Conditional shift: the empirical default rate plotted against $X$ is uniformly lower on the accepted slice than on the through-the-door population, even after conditioning on $X$. The shaded gap in (b) is the bias that reject inference must close; it cannot be removed by reweighting on $X$ alone because it comes from selection on the unobserved error, not from feature imbalance."
import numpy as np
import matplotlib.pyplot as plt
...
Panel (a) is what reweighting fixes: the feature distribution differs between funded and through-the-door, and inverse probability weights on $X$ recover $P(X)$ from $P(X \mid S=1)$. Panel (b) is what reweighting cannot fix: even at the same $X$, the accepted applicants default less, because the underwriter used signals that we never recorded and that also predict default. That is the part of the gap that motivates Heckman, the impossibility result, and everything that follows in this chapter.

Before tackling the methods in turn, it helps to map every stage where selection bias enters and every assumption a corrective method might lean on. @fig-ch10-selection-roadmap traces an applicant through the lender's pipeline and labels seven moderators that any reject inference exercise has to take an explicit position on. Each subsequent section in the chapter targets one or more of these moderators, and the impossibility result of @sec-ch10-impossibility says exactly which combinations the accepted-only sample can never settle on its own.

```{mermaid}
%%| label: fig-ch10-selection-roadmap
%%| fig-cap: "Selection pipeline (left) and the moderators every reject inference method has to take a position on (right). The applicant flows through screening, an underwriter decision, funding, and a performance window before $Y$ is observed. Bias enters at the underwriter stage as covariate shift on $X$ (M1), selection on unobservables that also drive default (M2), thin policy overlap (M3), and the availability of an exclusion restriction (M4); at the performance window as vintage and macro distortion of the observed default rate (M5); and at the bureau-surrogate step as product mismatch (M6) and within-reject coverage gaps (M7). Subsequent sections take the moderators one at a time."
flowchart LR
    classDef stage fill:#cfd8dc,stroke:#37474f,color:#111;
    classDef accept fill:#90caf9,stroke:#1565c0,color:#111;
    classDef reject fill:#ef9a9a,stroke:#c62828,color:#111;
    classDef obs fill:#1976d2,stroke:#0d47a1,color:#fff;
    classDef bureau fill:#f8bbd0,stroke:#ad1457,color:#111;
    classDef latent fill:#ede7f6,stroke:#4527a0,color:#111;
    classDef mod fill:#fff8e1,stroke:#b58900,color:#111;

    subgraph Pipe["Selection pipeline"]
        direction TB
        A["Through-the-door applicant<br/>X observed; Z partly observed;<br/>U, V latent"]:::stage
        S1["Stage 1: channel and eKYC screen<br/>eligibility, fraud, KYC"]:::stage
        LOST["Filtered before scoring;<br/>X often not stored"]:::latent
        S2["Stage 2: policy plus underwriter decision<br/>score cut on X, manual signals on V"]:::stage
        ACC["Funded loan:<br/>limit, rate, term"]:::accept
        REJ["Rejected; Y missing for the lender"]:::reject
        PW["Stage 3: performance window<br/>vintage v, macro state m"]:::stage
        Y["Outcome Y observed"]:::obs
        PRP["Prepay or refinance: competing event, ch 9"]:::latent
    ...
    subgraph Mods["Moderators every method must address"]
        direction TB
        M1["M1 covariate shift on X:<br/>P(X &#124; S=1) ≠ P(X)"]:::mod
        M2["M2 selection on unobservables:<br/>Corr(U, V) ≠ 0"]:::mod
        M3["M3 policy overlap:<br/>is P(S=1 &#124; x) > 0 everywhere?"]:::mod
        M4["M4 exclusion restriction:<br/>any Z that shifts S but not Y?"]:::mod
        M5["M5 vintage and macro state:<br/>through-the-cycle vs point-in-time"]:::mod
        M6["M6 bureau product gap:<br/>limit, rate, servicer differ"]:::mod
        M7["M7 within-reject coverage:<br/>10 to 30 percent have no trade-line"]:::mod
    end
...
A short reading guide to the diagram. Augmentation and parceling (@sec-ch10-augmentation-hsias-parceling-and-its-fuz) leans on M1 alone and assumes M2 away. Bureau extrapolation (@sec-ch10-bureau-extrapolation) buys M2 by importing $Y_B$ but inherits M5, M6, and M7. Heckman (@sec-ch10-heckman-selection-correction) trades M2 for parametric structure plus M4. AIPW, copulas, deep generative imputation, importance weighting, and PU learning (@sec-ch10-modern) each relax one Heckman primitive. Observable-engine methods (@sec-ch10-observable) attack M3 directly when the lender owns the decision engine. EM and pseudo-labeling (@sec-ch10-em) exploit cluster structure when none of the above is available.
...
beta_true = (-0.6, 1.2)
y_star = beta_true[0] + beta_true[1] * x + u
...
The accepted bad rate (around 0.28) sits well below the through-the-door rate (around 0.35). The 0.16 gap is the reject-inference target.
...
The accepted-only fit overstates the slope (around 2.23 versus a truth of 1.20) and pushes the intercept downward. Both directions match @eq-naive-target: with $\rho > 0$, conditioning on $S=1$ removes the high-$U$ tail among low-$x$ applicants, exaggerating how steeply default rises with $x$ inside the booked sample. We name this fitted curve $\hat p_A(x)$ and use it to score every applicant, including those who were rejected.
...
|------|--------------------|-------|------|-------------|-------|
| 1    | $(-\infty, 0.037)$ | 235   | 1    | 0.004       | 21    |
| 2    | $[0.037, 0.106)$   | 235   | 18   | 0.077       | 69    |
| 3    | $[0.106, 0.261)$   | 234   | 42   | 0.179       | 98    |
| 4    | $[0.261, 0.552)$   | 235   | 94   | 0.400       | 196   |
| 5    | $[0.552, +\infty)$ | 235   | 173  | 0.736       | 442   |

Two facts to notice. First, the bands span nearly the whole unit interval because the accepted-only PD fans out into a wide score distribution. Second, rejects pile up in band 5: 442 of the 826 rejects fall in the highest band, because the acceptance rule pushes high-$x$ applicants out, and high $x$ means high $\hat p_A$. This concentration is what makes parceling work or fail. If band 5 is large and its accepted bad rate ($\bar\pi_5 = 0.736$) is wrong as an estimate of the rejected bad rate in that band, the augmentation drags the refit toward a wrong target with a lot of weight.

**Step 3a: hard parceling.** For each band, randomly assign $\bar\pi_b N^R_b$ rejects to "bad" and the rest to "good". Band 5 contributes $0.736 \times 442 \approx 325$ synthetic bads. The augmented training set has 1174 accepteds with their real labels and 826 rejects with synthetic labels drawn from the band-specific Bernoulli. We do not implement hard parceling here because fuzzy is strictly preferable: fuzzy uses the same band rates but eliminates the sampling variance from the random draw of synthetic labels.
...
}, index=["truth", "oracle", "naive (acc only)", "fuzzy (tau=1)"])
print(compare.round(3))
```

**Trace one reject through the math.** Take a reject in band 4 with $x = 0.7$. Its accepted-only score is $\hat p_A(0.7) = \sigma(-0.685 + 2.233 \cdot 0.7) = \sigma(0.878) = 0.706$. That places the applicant in band 5 by score, but suppose for a clearer trace the applicant lands in band 4 with $\bar\pi_4 = 0.400$. The applicant's contribution to the refit log-likelihood is

$$
0.400 \log p(0.7; \beta) + 0.600 \log\big(1 - p(0.7; \beta)\big),
$$

which is the expectation of the log-likelihood under a $\mathrm{Bernoulli}(\bar\pi_4)$ draw at the same $x$. Treating $p(0.7; \beta)$ as a free probability, this is maximized at $p(0.7; \beta) = 0.400$, the band-4 accepted bad rate. The accepted-only fit puts $\hat p_A(0.7) = 0.706$ at this point, so the refit pulls the fitted curve at $x = 0.7$ downward toward 0.400. The slope at $x = 0.7$ flattens because the band rate is lower than the accepted-only fit at that score.

**Reading the comparison table.** The naive slope (around 2.23) is too steep relative to the oracle (around 2.09). The fuzzy refit pulls the slope down to roughly 1.70, which lands below the oracle. The intercept barely changes between naive and fuzzy. This is the @sec-ch10-impossibility result in miniature: fuzzy augmentation cannot recover the truth because its band rates are themselves a function of the biased $\hat p_A$. Whether the resulting bias overshoots or undershoots depends on which bands carry the most reject mass and whether $\bar\pi_b$ over- or under-estimates the rejected bad rate inside that band. Here, band 5 holds 442 of 826 rejects, and inside band 5 the oracle rejected bad rate is 0.498 while $\bar\pi_5 = 0.736$. The augmentation overweights bads in band 5 enough to overshoot the slope correction in the opposite direction.
...
The oracle reject bad rate per band is systematically higher than $\bar\pi_b$ in the low bands and lower in the highest band. That mismatch is the fingerprint of MNAR: even after conditioning on $\hat p_A$, rejects in any given band are not a random sample of the through-the-door population in that band. With real data the oracle column is unobservable, which is exactly why the impossibility result bites. Nothing in the augmentation procedure can detect or correct this gap from the accepted-only sample alone.

**What changes if we set** $\tau \neq 1$. Multiplying every $\bar\pi_b$ by a constant $\tau$ shifts the weights toward the "bad" row uniformly. This raises the refit's overall PD level (the intercept rises) but barely tilts the slope, because every band's rate moves by the same factor. A practitioner who knows from bureau pulls that the rejected population is roughly $\tau = 1.5$ times riskier than the same-band accepteds can use $\tau = 1.5$ as a level shift. There is no way to extract this $\tau$ from the accepted sample; it has to come in from outside, which is the topic of @sec-ch10-bureau-extrapolation.
...
}, index=["truth", "oracle", "naive (acc only)",
          "fuzzy (tau=1)", "bureau (w=0.7)"])
...
print(reliability(p_acc, y[s == 1]).to_string(index=False))
print("\nbureau-imputed rejects (Y^B):")
print(reliability(p_rej, y_rej_b).to_string(index=False))
```

If the two tables agree bin by bin, the surrogate is behaving like the contract outcome on average. If the bureau-imputed reliability runs systematically higher (predicted PD too low for the observed bureau default rate), the surrogate is picking up product-mix risk that the model is not parameterized to absorb; the right next steps are to add product-mix features ($Z^B$ such as bureau-product type, exposure relative to lender-A request, and time-to-first-trade-line) on the rejected side, or to lower $w_{\text{bureau}}$ until the calibration gap closes.
...
**Claim 1: the selection-equation variance is unidentified, so we set it to 1.** Suppose the latent selection equation were written with a free variance $\sigma_V^2$ rather than the normalization in @eq-heckman-sigma. The selection probit gives
$$
P(S = 1 \mid X, Z) = P(V > -X^\top \gamma_X - Z^\top \gamma_Z) = \Phi\!\left( \frac{X^\top \gamma_X + Z^\top \gamma_Z}{\sigma_V} \right).
$$
The observed data on $S$ only ever pin down the *ratio* $(\gamma_X, \gamma_Z) / \sigma_V$, never the numerator and denominator separately. Doubling all of $(\gamma_X, \gamma_Z, \sigma_V)$ leaves every acceptance probability unchanged, so the likelihood is flat along that ray. The standard fix is to set $\sigma_V = 1$ and read $(\gamma_X, \gamma_Z)$ on that scale. Any other scale convention (the most common alternative is $\sigma_V = \pi / \sqrt 3$, which makes the probit coefficients comparable to a logit) gives the same coefficients up to a uniform rescaling. The off-diagonal $\rho \sigma$ in @eq-heckman-sigma is the covariance, not the correlation, but with $\sigma_V = 1$ the covariance equals the correlation times $\sigma$, so the matrix entry is $\rho \sigma$ as written and $\rho$ is the recoverable correlation parameter.
...
$$
P(Y = 1 \mid X, Z, S = 1) = \Phi\!\left( \frac{X^\top \beta + \rho \sigma \, \lambda(a)}{\sigma \sqrt{1 - \rho^2 \, \delta(a)}} \right) = \Phi\!\left( \frac{X^\top \beta / \sigma + \rho \, \lambda(a)}{\sqrt{1 - \rho^2 \, \delta(a)}} \right).
$$
...
A1. **Bivariate normality of $(U, V)$.** The joint error is $\mathcal{N}(0, \Sigma)$ with $\Sigma_{12} = \rho$. *Diagnostic:* QQ-plot of standardized stage-2 residuals on the accepted sample, plus Shapiro-Wilk or Jarque-Bera on the residual tails. Heavy tails or skew motivate a Student-$t$ joint or a copula generalization (@sec-ch10-modern). In credit, gross income and bureau utilization are right-skewed; a log or Yeo-Johnson transform of the inputs usually closes most of the non-normality before the joint assumption is challenged.

A2. **Correct selection link.** $S_i = \mathbf{1}\{X_i^\top \gamma_X + Z_i^\top \gamma_Z + V_i > 0\}$ with $V_i$ standard normal. *Diagnostic:* the Pregibon (1980) link test on the stage-1 probit and a Hosmer-Lemeshow calibration test on $\hat P(S = 1)$. A misspecified link gives a wrong $\hat \lambda$ and biases stage 2 even when A1 holds. Banks whose policy is a logistic scorecard, not a normal-latent rule, should swap probit for the logit selection model with the @lee1983generalized substitute for $\lambda$.

A3. **Exclusion restriction: $Z$ enters selection but not the outcome residual.** *Diagnostic in two parts.* First, the strength check: F-statistic of $Z$ in the stage-1 probit, looking for $F > 10$ on the joint test, the same rule of thumb as for instrumental variables (@stock2002survey). Second, the exogeneity check: on a labelled subset of the rejected pool (typically bureau-labelled, see @sec-ch10-bureau-extrapolation), include $Z$ in the outcome equation directly and test that its coefficient is indistinguishable from zero. A nonzero coefficient kills the exclusion. Document the candidate $Z$ before fitting; ex-post search for a $Z$ that "works" is a known model-risk red flag.

A4. **Overlap: $0 < P(S = 1 \mid X = x, Z = z) < 1$ for every $(x, z)$ of interest.** *Diagnostic:* histogram of $\hat P(S = 1)$ stratified by $S$. If the rejected mass piles up below 1 percent or the accepted mass piles up above 99 percent, the policy is near-deterministic in part of feature space. The Hand-Henley impossibility (@sec-ch10-impossibility) bites in that region regardless of A1-A3, and $\hat\beta$ there is extrapolation under the parametric assumption. Trim or restrict inference to the overlap region; report the trimmed share in the model document.

A5. **Constant correlation $\rho$ across $(X, Z)$.** The sandwich in @eq-heckman-sandwich assumes a scalar $\rho$. In practice $\rho$ can differ between thin-file and thick-file applicants, or between branch and digital channels. *Diagnostic:* refit on disjoint subsamples (by channel, vintage, file thickness) and test equality of the IMR coefficient. Pooled $\hat\rho$ that masks heterogeneity hides the fact that one segment is MNAR while another is MAR, with direct consequences for the per-segment PD curve.

If A1-A5 are tenable, $\hat\beta_{\text{Heck}}$ is consistent for the through-the-door $\beta$. If any one fails, the bias is specific but generally not zero. The next two subsections cover how to *price* the residual uncertainty (variance) and how to *avoid* the assumptions altogether by changing the data-generating process (design-based estimation).
...
**(C) Operational and capacity instruments.** Underwriter identity, branch-level staffing shocks, system-downtime windows, queue position, weekend/holiday processing dummies. @dobbie2018consumer use bankruptcy-judge identity for Chapter 13 dismissal; @dobbie2017consumer extend the design to consumer credit outcomes. @stein2002information argues loan-officer hierarchy choices generate quasi-random variation in soft-information lending. Vietnam-specific candidate: Tet-period staffing reductions that compress decisioning capacity for a known applicant cohort. *Fragility:* if officer assignment correlates with borrower segment (specialist officers see specific products), excludability fails.
...
When selection is MAR, meaning $\rho = 0$, the coefficient on $\hat \lambda$ is zero and the Heckman estimator collapses to the naive fit. The natural alternative in that regime is inverse probability of selection weighting (IPW), and a thirty-year arc of refinements (Horvitz-Thompson normalization, augmented IPW with double robustness, double machine learning with cross-fitting) has produced increasingly flexible MAR estimators that the modern credit literature often treats as the state of the art. The relationship between this lineage and Heckman is the question of this subsection. The summary, derived below, is that DML and its relatives are strict upgrades of IPW on the functional-form axis but inherit the MAR ceiling on the selection axis, so they cannot replace Heckman when the underwriter selects on unobservables. Heckman trades a strong parametric joint assumption for genuine MNAR identification, and copula selection (@sec-ch10-modern) is the modern generalization on that second axis.
...
$$ {#eq-ht}
provided $\pi(x, z) > 0$ on the support of $(X, Z)$ and selection satisfies $S \perp\!\!\!\perp Y \mid (X, Z)$. The conditional independence is the formal statement of MAR. Under MAR, weighting each accepted observation by $1 / \pi$ recovers the through-the-door expectation of $h$ exactly. Specializing $h$ recovers familiar estimators: $h(Y, X) = \mathbf{1}\{Y = 1, X \in A\}$ gives the through-the-door PD on any region $A$; $h(Y, X) = -\log p(Y \mid X; \beta)$ gives the IPW M-estimator that recovers the through-the-door scorecard coefficients by maximum likelihood on the weighted accepted sample.
...
The raw Horvitz-Thompson estimator has unbounded variance whenever $\pi$ is small in part of feature space. In credit, the policy declines a substantial share of through-the-door volume and the rejected mass concentrates at low $\pi$. A handful of accepted observations with $\hat \pi_i \approx 0.02$ then dominate the weighted sum, and the estimator is volatile. The Hájek normalization divides through by the empirical sum of weights:
$$
\hat \mu_{\text{Hájek}} = \frac{\sum_i (S_i / \hat \pi_i) h(Y_i, X_i)}{\sum_i S_i / \hat \pi_i}.
$$ {#eq-hajek}
Hájek has the same asymptotic mean as Horvitz-Thompson but a smaller finite-sample variance whenever the propensity has heavy tails. In production we further clip $\hat \pi_i$ at a floor, typically 1 to 5 percent, and report the clipped share alongside the estimate. A clipped share above 5 percent is a hard overlap diagnostic: it means the policy is near-deterministic on the trimmed slice, the M3 moderator from @fig-ch10-selection-roadmap bites, and the IPW estimator is extrapolating along the parametric form of the propensity model rather than from data.
...
$$ {#eq-aipw-score}
Two algebraic facts make this score special, and both are direct calculations.

(1) If $\pi$ is correct (matches the true selection probability), then $\tilde Y$ is conditionally unbiased for $\mathbb{E}[Y \mid X]$ regardless of how $g$ is estimated. The augmentation cancels in expectation: $\mathbb{E}[\tilde Y \mid X] = g(X) + \mathbb{E}[(S / \pi)(Y - g(X)) \mid X] = g(X) + \mathbb{E}[Y \mid X] - g(X) = \mathbb{E}[Y \mid X]$ using the Horvitz-Thompson identity on the residual.

(2) Symmetrically, if $g$ is correct then $\tilde Y$ is conditionally unbiased even when $\pi$ is misspecified. The residual $Y - g(X)$ has zero conditional mean given $S = 1$, so the IPW augmentation contributes zero in expectation regardless of the weight.
...
A compact organizing picture separates the selection regime each estimator identifies from the functional form it imposes on the nuisances. The two axes are independent: an estimator's place on one says nothing about its place on the other.
...
DML is a strict upgrade over IPW on the functional-form axis: nuisance models can be nonparametric and inference is still $\sqrt{n}$-valid. It is not an upgrade on the selection axis. A lender whose underwriter conditions on signals never written to the feature store still needs the parametric joint of Heckman or an exclusion-based instrument to identify the through-the-door PD, and DML inherits the MAR ceiling regardless of how flexible the nuisance learners are. The genuine modern generalization of Heckman on the selection axis is copula selection (@marra2017bivariate, @sec-ch10-modern), which keeps the exclusion restriction but drops normality. Joint frailty (@sec-ch09) is the survival-time analog: censoring is selection on the time axis, IPCW is IPW on time, AIPCW is AIPW on time, and frailty plays the role of $\rho$ in the bivariate joint.
...
## Implementation from scratch {#sec-ch10-implementation-from-scratch}
...
class IVAudit:
    ...
    first_stage_F : float
        ...
        comparable to Stock and Yogo's rule of thumb of 10.
    ...
    def passes_strength(self, threshold: float = 10.0) -> bool:
...
print(f"First-stage F on Z (Stock-Yogo rule of thumb >= 10): {audit.first_stage_F:.2f}")
print(f"Strength test: {'PASS' if audit.passes_strength() else 'FAIL'}")
print("")
...
Under MAR, the through-the-door PD is identified by inverse-probability weighting:

$$
P(Y=1 \mid X=x) = \mathbb{E}\!\left[ \frac{S \cdot \mathbf{1}\{Y=1\}}{\pi(X, Z)} \,\bigg|\, X=x \right],
$$ {#eq-ipw-pd}

where $\pi(x, z) = P(S=1 \mid X=x, Z=z)$ is the propensity. The augmented IPW estimator of @robins1994estimation, refined for high-dimensional nuisance estimators by @chernozhukov2018double, is

$$
\hat \mu_{\text{DR}}(x) = g(x) + \frac{S}{\pi(x, z)}\big(Y - g(x)\big), \qquad g(x) = \mathbb{E}[Y \mid X=x, S=1].
$$ {#eq-aipw}

The key property is double robustness: $\hat \mu_{\text{DR}}$ is consistent for $P(Y=1 \mid X)$ if either $\pi$ or $g$ is correctly specified. Cross-fitting (split the sample, fit nuisances on one half and the score on the other) removes the small-sample bias of plug-in nuisance estimators and lets the second-stage estimator inherit a parametric $\sqrt{n}$ rate even when $\pi$ and $g$ are gradient-boosted trees or neural nets.

We implement AIPW for a logistic PD by constructing a pseudo-outcome $\tilde Y_i = g_i + (S_i / \pi_i)(Y_i - g_i)$ on the full applicant sample, clipping to $[0, 1]$, and refitting a weighted logistic. Cross-fitting splits the sample into five folds.
...
print(pd.DataFrame({
    ...
    "oracle":    np.concatenate([oracle_logit.intercept_, oracle_logit.coef_[0]]),
    "naive":     np.concatenate([pd_naive.intercept_, pd_naive.coef_[0]]),
    "aipw":      np.concatenate([aipw_mod.intercept_, aipw_mod.coef_[0]]),
...
@marra2017bivariate and @marra2013simultaneous generalize Heckman by replacing the bivariate-normal joint of $(U, V)$ with an arbitrary copula family. Identification still rests on the exclusion restriction, but the dependence between selection and outcome can be heavy-tailed (Student-$t$), asymmetric (Clayton, Gumbel), or radially symmetric without normality (Frank). For probit margins on both equations, the joint cell probabilities follow from the copula CDF $C_\theta(u, v)$:
...
print(pd.DataFrame({
    "true":          np.concatenate([beta_true, [np.nan]]),
...
print(pd.DataFrame({
    ...
    "oracle":    np.concatenate([oracle_logit.intercept_, oracle_logit.coef_[0]]),
    "naive":     np.concatenate([pd_naive.intercept_, pd_naive.coef_[0]]),
    "vae_like":  np.concatenate([gri_logit.intercept_, gri_logit.coef_[0]]),
}, index=["intercept", "X1", "X2"]).round(3))
```
...
print(pd.DataFrame({
    ...
    "oracle":    np.concatenate([oracle_logit.intercept_, oracle_logit.coef_[0]]),
    "naive":     np.concatenate([pd_naive.intercept_, pd_naive.coef_[0]]),
    "covshift":  np.concatenate([csa_logit.intercept_, csa_logit.coef_[0]]),
}, index=["intercept", "X1", "X2"]).round(3))
```
...
The methods above all treat the acceptance rule as unobserved. The lender sees $(X, Z, S)$ and infers a propensity model. In practice some firms observe the decision engine itself: a fintech that runs a deterministic logistic model with logged coefficients, a bank with a documented overlay matrix, a marketplace lender that records the platform's underwriting score and the investor selection on top of it. When the engine is observable, the propensity is not estimated from data; it is read from the model registry. Most of the @sec-ch10-modern toolbox simplifies sharply when it applies.

Three regimes are common in production:

1.  *Stochastic policy with logged probabilities.* Many fintechs randomize at the margin (a 5 to 10 percent random-approval quota for monitoring, or a Thompson-sampling bandit for active learning). The system logs $\pi(x_i, z_i)$ for every applicant. Use it directly.
2.  *Deterministic cutoff on a known score.* The classic pattern: $S_i = \mathbf{1}\{\text{score}(X_i, Z_i) > \tau\}$, where the score function is a known model and $\tau$ is the documented threshold. Regression-discontinuity (@thistlethwaite1960regression, @hahn2001identification, @imbens2008recent) recovers the local PD on either side of $\tau$ without any parametric joint.
3.  *Multi-stage deterministic gates.* Pre-gate (auto-decline on bureau score), then policy overlay (DTI cap, fraud screen), then judgmental review. Each gate is a conditional acceptance probability that the modeler can compose into an exact propensity.
...
print(pd.DataFrame({
    ...
    "oracle":      np.concatenate([oracle_logit.intercept_, oracle_logit.coef_[0]]),
    "naive":       np.concatenate([pd_naive.intercept_, pd_naive.coef_[0]]),
    "ipw_exact":   np.concatenate([exact_ipw_logit.intercept_, exact_ipw_logit.coef_[0]]),
    "aipw_exact":  np.concatenate([aipw_known_logit.intercept_, aipw_known_logit.coef_[0]]),
    "aipw_estim":  np.concatenate([aipw_mod.intercept_, aipw_mod.coef_[0]]),
}, index=["intercept", "X1", "X2"]).round(3))
```
...
print(pd.DataFrame({
    "true_beta":      beta_true,
    "oracle":         np.concatenate([oracle_logit.intercept_, oracle_logit.coef_[0]]),
    "aipw_composed":  np.concatenate([aipw_comp_logit.intercept_, aipw_comp_logit.coef_[0]]),
}, index=["intercept", "X1", "X2"]).round(3))
```
...
print(pd.DataFrame({
    "true_beta":     beta_true,
    "oracle":        np.concatenate([oracle_logit.intercept_, oracle_logit.coef_[0]]),
    "cfrm_loose":    np.concatenate([cfrm_logit.intercept_, cfrm_logit.coef_[0]]),
}, index=["intercept", "X1", "X2"]).round(3))
...
%%| fig-cap: "Decision tree for selecting a reject-inference method. Each branch asks a single yes/no question about the data the lender has logged. Terminal nodes point at the chapter section that develops the method and a one-line operational summary. The tree is read top-down: identify the layer, ask whether the engine is observable, ask whether an exclusion restriction is available, and so on. The methods at the bottom are not mutually exclusive; in production stacks, each terminal method becomes one input to a sensitivity analysis."
flowchart TD
    ...
    L4B -->|Yes| M4C["Heckman two-step<br/>(@sec-ch10-heckman-selection-correction)"]:::method
...
The cheat sheet in the table below pairs the most common production scenarios with the right method and the data prerequisite. A lender starting from scratch can read it as a roadmap for the data-engineering investments that unlock each method.
...
The survival chapter (@sec-ch09) treats informative censoring (selection on the latent event time $T$) as an analog of MNAR selection on $Y$. The two problems share a missing-data taxonomy and most methods translate one-for-one. The mapping is:
