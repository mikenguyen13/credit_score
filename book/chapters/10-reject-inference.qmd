...
**The case-by-case fix table.** A model-risk team reading the per-source panel runs the following decision tree against each source. The fix is mechanical once the panel and the registry are in place.
...
@fig-ch10-rho-positive and @fig-ch10-rho-negative make the mechanism concrete by listing example unobservables that would generate each sign of $\hat\rho$ in a Vietnamese consumer-finance setting. The dashed circles are the latent shocks $U$ (default) and $V$ (acceptance) of @eq-latent-default and @eq-latent-selection; the curved double arrow is the cross-equation correlation. Each bullet is a single latent trait whose joint footprint on $V$ and $U$ explains why $\hat\rho$ takes the sign claimed.

```{python}
#| label: fig-ch10-rho-positive
#| fig-cap: "Positive $\\hat\\rho$: the underwriter accepts on a latent trait that *also* drives default. Each example unobservable raises $V$ (the underwriter's acceptance shock) and $U$ (the borrower's default shock) at the same time, so the latent traits feed both circles in the same direction. With $\\rho \\sigma > 0$ in @eq-heckman-outcome, the conditional mean correction $\\rho \\sigma \\hat\\lambda(a)$ is added on top of $X^\\top \\beta$ on the accepted slice, so the accepted-only PD curve sits above the through-the-door curve unless the Heckman correction is applied."
import matplotlib.pyplot as plt
...
ax.text(5.0, 5.65,
        r"Positive $\rho$: underwriter accepts on a trait that $\it{also}$ drives default",
        ha="center", va="center", fontsize=12.5, fontweight="bold", color="#111")

u_center = (1.7, 4.2); v_center = (8.3, 4.2); r = 0.7
ax.add_patch(Circle(u_center, r, facecolor="#ffebee",
                    edgecolor="#c62828", linewidth=2.0, linestyle="--"))
ax.add_patch(Circle(v_center, r, facecolor="#e3f2fd",
                    edgecolor="#1565c0", linewidth=2.0, linestyle="--"))
ax.text(u_center[0], u_center[1] + 0.12, "$U$",
        ha="center", va="center", fontsize=20, color="#c62828", fontweight="bold")
ax.text(u_center[0], u_center[1] - 0.42, "latent\ndefault shock",
        ha="center", va="center", fontsize=8, color="#c62828")
ax.text(v_center[0], v_center[1] + 0.12, "$V$",
        ha="center", va="center", fontsize=20, color="#1565c0", fontweight="bold")
ax.text(v_center[0], v_center[1] - 0.42, "latent\nacceptance shock",
        ha="center", va="center", fontsize=8, color="#1565c0")

ax.add_patch(FancyArrowPatch(
    (u_center[0] + 0.55, u_center[1] + 0.4),
    (v_center[0] - 0.55, v_center[1] + 0.4),
    connectionstyle="arc3,rad=-0.35",
    arrowstyle="-|>", mutation_scale=20, lw=2.2, color="#2e7d32"))
ax.add_patch(FancyArrowPatch(
    (v_center[0] - 0.55, v_center[1] - 0.4),
    (u_center[0] + 0.55, u_center[1] - 0.4),
    connectionstyle="arc3,rad=-0.35",
    arrowstyle="-|>", mutation_scale=20, lw=2.2, color="#2e7d32"))
ax.text(5.0, 5.05, r"$\rho > 0$",
        ha="center", va="center", fontsize=20, color="#1b5e20", fontweight="bold")
ax.text(5.0, 3.4, r"$\mathrm{Corr}(U, V) > 0$",
        ha="center", va="center", fontsize=10.5, color="#1b5e20")

ax.text(5.0, 2.95,
        "example unobservables: each raises $V$ (accept) and $U$ (default) together",
        ha="center", va="center", fontsize=10, fontstyle="italic", color="#37474f")

examples_pos = [
    ("Persuasive narrative & charisma",
     r"officer trusts the story $\Rightarrow V\uparrow$;  same overconfidence drives leverage demand $\Rightarrow U\uparrow$"),
    ("Headline informal-cash income",
     r"officer rewards the big stated number $\Rightarrow V\uparrow$;  cash-flow volatility drives delinquency $\Rightarrow U\uparrow$"),
    ("Branch-officer relationship / soft signal",
     r"relationship lender accepts $\Rightarrow V\uparrow$;  social-network pressure masks family-debt overhang $\Rightarrow U\uparrow$"),
]
for i, (trait, why) in enumerate(examples_pos):
    y = 2.4 - 0.5 * i
    ax.text(0.45, y, f"• {trait}",
            ha="left", va="center", fontsize=9.5, fontweight="bold", color="#37474f")
    ax.text(4.3, y, why, ha="left", va="center", fontsize=8.5, color="#546e7a")

ax.add_patch(FancyBboxPatch((0.4, 0.05), 9.2, 0.6,
...
#| fig-cap: "Negative $\\hat\\rho$: the underwriter screens out latent risk effectively. Each example unobservable moves $V$ and $U$ in *opposite* directions, so the latent traits that the underwriter rewards (raising $V$) are the same traits that lower default risk (lowering $U$), and conversely the latent traits the underwriter penalises (lowering $V$) are the same traits that raise default risk. With $\\rho \\sigma < 0$ in @eq-heckman-outcome, the conditional mean correction $\\rho \\sigma \\hat\\lambda(a)$ is subtracted from $X^\\top \\beta$ on the accepted slice, so the accepted-only PD curve sits below the through-the-door curve unless the Heckman correction is applied."
import matplotlib.pyplot as plt
from matplotlib.patches import Circle, FancyArrowPatch, FancyBboxPatch
...
ax.text(5.0, 5.65,
        r"Negative $\rho$: underwriting policy screens out the latent risk",
        ha="center", va="center", fontsize=12.5, fontweight="bold", color="#111")
...
ax.add_patch(FancyArrowPatch(
    ...
    (v_center[0] - 0.55, v_center[1] + 0.4),
    connectionstyle="arc3,rad=-0.35",
    arrowstyle="-|>", mutation_scale=20, lw=2.2, color="#ad1457"))
ax.add_patch(FancyArrowPatch(
    (v_center[0] - 0.55, v_center[1] - 0.4),
    (u_center[0] + 0.55, u_center[1] - 0.4),
    connectionstyle="arc3,rad=-0.35",
    arrowstyle="-|>", mutation_scale=20, lw=2.2, color="#ad1457"))
ax.text(5.0, 5.05, r"$\rho < 0$",
        ha="center", va="center", fontsize=20, color="#880e4f", fontweight="bold")
ax.text(5.0, 3.4, r"$\mathrm{Corr}(U, V) < 0$",
        ha="center", va="center", fontsize=10.5, color="#880e4f")

ax.text(5.0, 2.95,
        "example unobservables: $V$ and $U$ move in opposite directions",
        ha="center", va="center", fontsize=10, fontstyle="italic", color="#37474f")

examples_neg = [
    ("Documentation completeness",
     r"officer rewards a clean file $\Rightarrow V\uparrow$;  same conscientiousness lowers delinquency $\Rightarrow U\downarrow$"),
    ("Verifiable formal-employer letter",
     r"officer trusts payroll proof $\Rightarrow V\uparrow$;  job stability lowers default $\Rightarrow U\downarrow$"),
    ("Underwriter notes flag a fraud signal",
     r"officer rejects on the red flag $\Rightarrow V\downarrow$;  same red flag predicts default if funded $\Rightarrow U\uparrow$"),
]
for i, (trait, why) in enumerate(examples_neg):
    y = 2.4 - 0.5 * i
    ax.text(0.45, y, f"• {trait}",
            ha="left", va="center", fontsize=9.5, fontweight="bold", color="#37474f")
    ax.text(4.3, y, why, ha="left", va="center", fontsize=8.5, color="#546e7a")
...
**Numerical fingerprint.** The identification claim is testable on a single simulation. Vary $\sigma$ in the data-generating process and re-fit the probit Heckman: the second-stage coefficient on $\hat \lambda$ should be invariant to $\sigma$ because it estimates $\rho$ alone. In the linear case, the same coefficient varies linearly with $\sigma$ because it estimates $\rho \sigma$. The block below makes that difference visible. We hold $\rho = 0.5$ fixed, sweep $\sigma$ over four values, and refit the same two-step estimator on a continuous outcome (linear stage 2) and a thresholded binary outcome (probit stage 2) drawn from the *same* latent process. The probit column hovers around $\rho$ within Monte Carlo noise; the linear column scales linearly in $\sigma$.
...
for sigma in [0.5, 1.0, 2.0, 4.0]:
    ...
    # stage 2 (linear-Y): OLS coef on lambda is consistent for rho * sigma
    coef_lin = sm.OLS(Y_lin[acc], Wacc).fit().params[-1]

    # stage 2 (probit-Y): probit coef on lambda is consistent for rho alone
    coef_pro = sm.Probit(Y_bin[acc], Wacc).fit(disp=False).params[-1]

    records.append({
        "sigma":         sigma,
        "linear_lam":    round(coef_lin, 3),
        "linear_target": round(rho_fp * sigma, 3),
        "probit_lam":    round(coef_pro, 3),
        "probit_target": round(rho_fp, 3),
    })

print(pd.DataFrame(records).to_string(index=False))
```

Read the table column by column. The `linear_lam` estimate doubles when $\sigma$ doubles and tracks `linear_target` $= \rho \sigma$ to within Monte Carlo noise; the linear two-step is identifying the *product* $\rho \sigma$ and cannot tell apart "moderate selection on unobservables, high outcome noise" from "strong selection on unobservables, low outcome noise". The `probit_lam` estimate sits in a tight band around `probit_target` $= \rho = 0.5$ across the same sweep (small drift at the extremes is finite-sample noise from the shifting accept rate), because the binary outcome equation absorbs $\sigma$ into the latent-scale normalization and what survives is $\rho$ alone. The whole table is generated from one set of $(V, \eta, X, Z)$ draws; only $\sigma$ is rescaled between rows, so the contrast is not a sampling artifact. The same fingerprint is what tells a model risk team that a probit-stage-2 Heckman fit on a real PD problem is reporting the underlying selection correlation, not a quantity confounded with the unidentified outcome variance.
...
4.  Fit the outcome regression of $Y$ on $(X, \hat r)$ on the accepted sample only. The link can be probit, logit, or linear depending on the deployment target. The coefficient on $\hat r$ is an estimate of $\rho^{*} \sigma$ (or $\rho^{*}$ in the probit-outcome case with $\sigma = 1$); it is not directly $\rho$ on the original $(U, V)$ scale because the marginals were remapped.

5.  Standard errors require either a sandwich correction that propagates the stage-1 logit uncertainty into the stage-2 coefficients, or a cluster bootstrap that resamples applicants (or applicant-vintage clusters in production). The sandwich derivation is mechanically the same as in @sec-ch10-heckman-variance, with the logistic score and information replacing the probit ones; we implement both in @sec-ch10-heckman-se-impl.
...
**Decision rule for production teams.**
...
The role of $h$ deserves a moment of unpacking, because the rest of this subsection treats it as a slot to be filled rather than a fixed object. A *functional* in this context is any map from a random variable to a number: pick a function of $(Y, X)$, take its expectation under the through-the-door distribution, and you have an estimand. The same Horvitz-Thompson identity covers all of them simultaneously, which is why the chapter states it for an arbitrary $h$ rather than separately for PD, log-likelihood, and dollar loss. Beyond the two specializations just listed, three further choices show up in production. First, through-the-door expected loss, $h(Y, X) = Y \cdot \text{EAD}(X) \cdot \text{LGD}(X)$, gives the dollar loss per applicant on the full pool rather than on the funded slice. Second, the calibration moment in score bin $b$, $h(Y, X) = (Y - \hat p(X)) \mathbf{1}\{\hat p(X) \in b\}$, tests whether the score is calibrated against the through-the-door default rate; the unweighted accept-only analog calibrates trivially because the policy itself selects on score, so calibration on accepts is a property of the policy rather than the score. Third, the feature mean $h(Y, X) = X_j$ does not involve $Y$ and can be computed directly on the full applicant pool without weighting, which turns it into a free diagnostic on $\hat\pi$: a weighted accepted mean that fails to match the directly-computed pool mean indicates a miscalibrated propensity. The same generality propagates to AIPW (next subsection). Replace $Y$ with $h(Y, X)$ and the outcome regression $g(X) = \mathbb{E}[Y \mid X, S = 1]$ with $g_h(X) = \mathbb{E}[h(Y, X) \mid X, S = 1]$, and the doubly-robust score, the Neyman-orthogonality argument, and the cross-fitting recipe carry over verbatim. The two-axis taxonomy in the table below is therefore a statement about each estimator's identification properties, not about which target $h$ it can address.
