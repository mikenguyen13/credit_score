...
for i in range(N_FIRM):
    ...
    pd_i = 1 / (1 + np.exp(-(-3.0 + 4.0 * lr + 0.4 * rng.standard_normal())))
...
for col in ["neg", "pos", "unc", "net"]:
    rho = np.corrcoef(panel[col], panel["pd"])[0, 1]
...
```{python}
from csutils import load_german_credit, gini
