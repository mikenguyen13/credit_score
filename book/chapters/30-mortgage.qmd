...
def cox_partial_ll(beta, X, events, times):
    ...
    eta = X_s @ beta
    # Cumulative log-sum-exp over the risk set. The naive form
    # log(cumsum(exp(eta))) overflows for any |eta| >> 700; the
    # accumulate form uses the log1p+abs identity at each step and
    # never instantiates a value larger than max(eta).
    log_cum = np.logaddexp.accumulate(eta)
