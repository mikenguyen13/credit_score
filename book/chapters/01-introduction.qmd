...
#| fig-cap: "Side-by-side comparison of the two benchmark datasets. Left: sample size on a log scale (the Taiwan set is thirty times larger than the German set). Middle: feature count. Right: default rate, with the Statlog German set oversampled to 30 percent and the Taiwan set at its natural 22 percent portfolio rate."
names = ['German', 'Taiwan']
sizes = [len(german), len(taiwan)]
feats = [german.shape[1] - 1, taiwan.shape[1] - 1]
rates = [german['default'].mean(), taiwan['default'].mean()]
...
axes[1].bar(names, feats, color=colors)
...
```{python}
#| label: fig-split-sizes
#| fig-cap: "Row counts for each slice produced by `train_valid_test_split` at `seed=0`. The 60/20/20 rule is proportional, so the German set (1,000 rows) and the Taiwan set (30,000 rows) get the same *shares* but very different *absolute* sample sizes for validation and test. A 200-row German test slice is small enough that a single metric estimate carries wide confidence intervals, which motivates the resampling designs in later chapters."
split_rows = []
for name, df in [('German', german), ('Taiwan', taiwan)]:
    tr, va, te = train_valid_test_split(df, y_col='default', seed=0)
    split_rows.append((name, len(tr), len(va), len(te)))

labels = [r[0] for r in split_rows]
n_tr = np.array([r[1] for r in split_rows])
n_va = np.array([r[2] for r in split_rows])
n_te = np.array([r[3] for r in split_rows])
totals = n_tr + n_va + n_te
slice_colors = {'train': '#4C72B0', 'valid': '#55A868', 'test': '#C44E52'}

fig, ax = plt.subplots(figsize=(8, 2.8))
ypos = np.arange(len(labels))
ax.barh(ypos, n_tr, color=slice_colors['train'], label='train (60%)')
ax.barh(ypos, n_va, left=n_tr, color=slice_colors['valid'], label='valid (20%)')
ax.barh(ypos, n_te, left=n_tr + n_va, color=slice_colors['test'], label='test (20%)')

for i, (tot, a, b, c) in enumerate(zip(totals, n_tr, n_va, n_te)):
    ax.text(a / 2, i, f'{a:,}', ha='center', va='center',
            color='white', fontsize=9)
    ax.text(a + b / 2, i, f'{b:,}', ha='center', va='center',
            color='white', fontsize=9)
    ax.text(a + b + c / 2, i, f'{c:,}', ha='center', va='center',
            color='white', fontsize=9)
    ax.text(tot, i, f'  n={tot:,}', ha='left', va='center', fontsize=9)
