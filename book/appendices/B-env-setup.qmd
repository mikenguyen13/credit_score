...
**Fairness.** `fairlearn`, `aif360`. Demographic parity, equalised odds, and reweighting. Used in the fairness chapters.
...
On MPS, watch for float64 operations. MPS supports float32 and float16. Cast explicitly before sending tensors to the device. On CUDA, check `torch.cuda.mem_get_info()` before loading 7B-parameter LLMs; the LLM chapter uses 8-bit quantisation via `bitsandbytes` to fit on a 24GB card.
...
`csutils._cache_get` implements the caching logic. The function is a dozen lines:
...
This controls the randomisation of hashes for strings, bytes, and several other types. Without it, dict iteration order differs run-to-run for tie-breaking paths that hash values.
...
BLAS reductions are not associative in float arithmetic. Different thread counts compute partial sums in different orders, which changes the last few ULPs of the result. For model monitoring (PSI over time), those ULPs are irrelevant. For bit-for-bit reproduction of a regulatory artefact, they matter.
...
`warn_only=True` trades determinism for fallback on ops that have no deterministic kernel. For regulatory artefacts, set it to `False` and accept that some ops will raise. You then have to rewrite the forward pass to avoid them.
...
A container lets you hand a validator a single artefact that builds the book end to end. The Dockerfile below uses a multi-stage pattern. Stage one resolves dependencies with `uv`. Stage two renders the book with Quarto.
...
2. Import helpers with `sys.path.insert(0, '../code'); from csutils import ...`. Do not copy helper functions into the chapter.
...
**`ModuleNotFoundError: No module named 'csutils'`.** The chapter was rendered from outside the project root. `execute-dir: project` in `_quarto.yml` sets the working directory, but only when you run `quarto render` from the root.
